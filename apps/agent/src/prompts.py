...
GENERATIVE_UI_PROMPT = (
    ...
    "  human-in-the-loop draft outreach email. Call this WHENEVER the user\n"
    "  says \"draft / write / compose / send an email to <name>\" or asks for\n"
    "  an outreach message for a specific lead. Resolve <name> by calling\n"
    ...
    "  card is a review surface: the user can edit subject/body inline and\n"
    "  picks Approve / Regenerate / Discard / change-tone. Each button\n"
    "  round-trips a follow-up prompt back to you with the user's final\n"
    "  edits — DO NOT also write the email body in plain prose, and DO NOT\n"
    "  queue or send in the same turn as renderEmailDraft. Wait for the\n"
    "  approval round-trip.\n"
    "  ON APPROVAL: when the follow-up prompt says the user approved the\n"
    "  draft, call `post_outreach_comment(leadId, subject, body)` — pass\n"
    "  the user's FINAL subject and body verbatim (the prompt includes\n"
    "  them as JSON-quoted strings). Do not paraphrase. After the tool\n"
    "  returns, give a one-line confirmation in chat. ON DISCARD: just\n"
    "  acknowledge briefly and stop. ON REGENERATE / tone change: emit a\n"
    "  fresh renderEmailDraft with the new draft.\n"
